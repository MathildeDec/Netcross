"""Remplit les .po en_US avec les traductions anglaises."""
import re

# Dictionnaire de traduction fr -> en pour GTK4
gtk4_translations = {
    ...
    "Seuil pertes (points de %):": "Loss threshold (percentage points):",
...
translations_map = {
    "netcross-gtk4": gtk4_translations,
    "netcross-report": report_translations,
    "netcross-cli": cli_translations,
}

for domain, translations in translations_map.items():
    ...
    while i < len(lines):
        ...
        if line.startswith('msgid "'):
            ...
            if i + 1 < len(lines) and lines[i + 1].strip() == 'msgstr ""':
                en = translations.get(msgid, msgid)
                result.append(f'msgstr "{en}"')
                filled += 1
                i += 2
                continue
        result.append(line)
    ...
    print(f"  {domain}: {filled} msgstr remplis")
