...
Report.exploit_suspicion_flows ───────────────────► anomaly_findings ─────────────┼► Report.security_findings
service/version ──► security.correlate_banner ────► cve_findings ─────────────────┘            │
                                                                                                ▼
                                                              netcross_report.security_report (rendu texte)
```

`netcross_core.security.findings.apply_security_findings()` assemble les trois listes de
